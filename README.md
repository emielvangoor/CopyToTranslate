...
- **Select and press §.** With Accessibility permission, the shortcut reads selected text directly. On-device classification routes Spanish to English translation and Dutch to proofreading. With no selection, it uses the clipboard.
...
4. Select text in your app, then press the bare **§** key. Spanish is translated to English on-device; Dutch is corrected through OpenRouter. No Command, Option or Control is needed. With nothing selected, the same classifier uses your clipboard. The menu also offers **Translate or Correct Selection (§)** and explicit Spanish/Dutch clipboard commands.
...
Dutch proofreading is explicit and still works while automatic Spanish translation is paused. It skips likely code, names, fragments and uncertain language matches. Selected text is read through macOS Accessibility only on request, without copying it or reading the entire document. Password fields are rejected. If permission is missing or an app does not expose its selection, a visible card points to Setup or the explicit clipboard menu commands; it does not send unrelated clipboard text. The clipboard commands work without Accessibility permission. Ollama and a local model download are no longer needed.
